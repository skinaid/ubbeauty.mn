...
type ConfirmRequest = { display: string; locationData: Partial<ClinicLocation> & { id?: string } };

export function LocationsChatPanel({
  orgId,
  locations,
  onLocationUpdate,
}: {
  ...
  onLocationUpdate: (loc: ClinicLocation) => void;
}) {
  void orgId;
  ...
  useEffect(() => {
    ...
    setMessages([{
      ...
      role: "assistant",
      content: count === 0
        ? "Сайн байна уу! Эмнэлгийн салбаруудын байршлыг бүртгэцгээе. Эхний салбарын нэр, хаягийг хэлнэ үү 📍"
        : `Одоо ${count} салбар бүртгэлтэй байна. Шинэ салбар нэмэх эсвэл байгаа салбарын мэдээллийг засах боломжтой.`,
    }]);
  ...
  const doStream = useCallback(async (userMsg: Message, prevMessages: Message[]) => {
    ...
    try {
      const allMessages = [...prevMessages, userMsg].map((m) => ({ role: m.role, content: m.content }));
      const res = await fetch("/api/clinic/locations-agent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages: allMessages, existingLocations: locations }),
      });
        ...
        for (const line of lines) {
          ...
          try {
            const event = JSON.parse(line.slice(6)) as {
              ...
              content?: string;
              display?: string;
              locationData?: Partial<ClinicLocation> & { id?: string };
            ...
            } else if (event.type === "confirm_location" && event.display && event.locationData) {
              setPendingConfirm({ display: event.display, locationData: event.locationData });
            } else if (event.type === "location_saved" && event.location) {
              onLocationUpdate(event.location);
            }
          } catch { /* skip */ }
        }
      }
  ...
  const handleConfirm = useCallback(async (confirmed: boolean) => {
    ...
    if (!confirmed) {
      setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "За, өөрчлүүлээрэй." }]);
      return;
    }
    ...
    try {
      const res = await fetch("/api/clinic/locations-agent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ directSave: confirm.locationData }),
      });
      ...
      while (true) {
        ...
        for (const line of lines) {
          if (!line.startsWith("data: ")) continue;
          try {
            const event = JSON.parse(line.slice(6)) as { type: string; content?: string; location?: ClinicLocation };
            if (event.type === "text" && event.content) {
  ...
  }, [pendingConfirm, onLocationUpdate]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#fafafa" }}>
      <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid #e5e7eb" }}>
        <p style={{ fontSize: "0.7rem", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", margin: 0 }}>AI Байршил оруулагч</p>
      </div>
      ...
      <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid #e5e7eb", background: "#ffffff" }}>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-end" }}>
          <textarea
            ...
            placeholder={pendingConfirm ? "Баталгаажуулалтыг эхлээд шийднэ үү..." : "Салбарын нэр, хаяг хэлнэ үү..."}
            rows={2}
