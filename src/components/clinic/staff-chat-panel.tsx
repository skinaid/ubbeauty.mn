...
type ConfirmRequest = { display: string; staffData: Partial<StaffMember> & { id?: string } };
...
export function StaffChatPanel({
  ...
  orgId: string;
  staff: StaffMember[];
  onStaffUpdate: (s: StaffMember) => void;
}) {
  void orgId;
    ...
    setMessages([
      {
        ...
        content:
          staff.length === 0
            ? "Сайн байна уу! Эмнэлгийн ажилтнуудыг бүртгэцгээе. Ажилтны нэр, мэргэжил, үүргийг хэлнэ үү 👤"
            : `Одоо ${staff.length} ажилтан бүртгэлтэй. Шинэ ажилтан нэмэх буюу засах боломжтой.`,
      },
    ]);
  ...
  const doStream = useCallback(
    async (userMsg: Message, prevMessages: Message[]) => {
      setIsStreaming(true);
      setPendingConfirm(null);
      const assistantId = (Date.now() + 1).toString();
      setMessages((prev) => [...prev, { id: assistantId, role: "assistant", content: "" }]);
      try {
        const res = await fetch("/api/clinic/staff-agent", {
          method: "POST",
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              } else if (event.type === "confirm_staff" && event.display && event.staffData) {
                setPendingConfirm({ display: event.display, staffData: event.staffData });
              } else if (event.type === "staff_saved" && event.staff) {
                onStaffUpdate(event.staff);
              }
            } catch {
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === assistantId ? { ...m, content: "Алдаа гарлаа." } : m))
        );
      } finally {
  ...
  const handleConfirm = useCallback(
    async (confirmed: boolean) => {
      ...
      const userMsg: Message = {
        ...
        role: "user",
        content: confirmed ? "✓ Тийм" : "Үгүй",
      };
      setMessages((prev) => [...prev, userMsg]);
      if (!confirmed) {
        setMessages((prev) => [
          ...prev,
          { id: (Date.now() + 1).toString(), role: "assistant", content: "За, өөрчлүүлээрэй." },
        ]);
        return;
      }
      setIsStreaming(true);
      ...
      try {
        const res = await fetch("/api/clinic/staff-agent", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ directSave: confirm.staffData }),
        });
      ...
      <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid #e5e7eb", background: "#ffffff" }}>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-end" }}>
          <textarea
            ...
            placeholder={
              pendingConfirm
                ? "Баталгаажуулалтыг эхлээд шийднэ үү..."
                : "Ажилтны нэр, мэргэжил, үүрэг хэлнэ үү..."
            }
            rows={2}
            style={{
              flex: 1,
