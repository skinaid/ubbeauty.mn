...
type ConfirmRequest = { display: string; serviceData: Partial<Service> & { id?: string } };

export function ServicesChatPanel({ orgId, services, onServiceUpdate }: {
  orgId: string; services: Service[]; onServiceUpdate: (s: Service) => void;
}) {
  void orgId;
  ...
  useEffect(() => {
    setMessages([{ id: "init", role: "assistant", content: services.length === 0 ? "Сайн байна уу! Эмнэлгийн үйлчилгээнүүдийг бүртгэцгээе. Үйлчилгээний нэр, хугацаа, үнийг хэлнэ үү 💆" : `Одоо ${services.length} үйлчилгээ бүртгэлтэй. Шинэ үйлчилгээ нэмэх боломжтой.` }]);
  }, []);
  ...
  const doStream = useCallback(async (userMsg: Message, prevMessages: Message[]) => {
    ...
    try {
      const res = await fetch("/api/clinic/services-agent", {
        ...
        body: JSON.stringify({ messages: [...prevMessages, userMsg].map((m) => ({ role: m.role, content: m.content })), existingServices: services }),
      });
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            const event = JSON.parse(line.slice(6)) as { type: string; content?: string; display?: string; serviceData?: Partial<Service> & { id?: string }; service?: Service };
            if (event.type === "text" && event.content) setMessages((prev) => prev.map((m) => m.id === assistantId ? { ...m, content: m.content + event.content! } : m));
            else if (event.type === "confirm_service" && event.display && event.serviceData) setPendingConfirm({ display: event.display, serviceData: event.serviceData });
            else if (event.type === "service_saved" && event.service) onServiceUpdate(event.service);
          } catch { /* skip */ }
        }
      }
  ...
  const handleConfirm = useCallback(async (confirmed: boolean) => {
    ...
    try {
      const res = await fetch("/api/clinic/services-agent", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ directSave: confirm.serviceData }),
      });
      ...
      while (true) {
        ...
        for (const line of lines) {
          if (!line.startsWith("data: ")) continue;
          try {
            const event = JSON.parse(line.slice(6)) as { type: string; content?: string; service?: Service };
            if (event.type === "text" && event.content) setMessages((prev) => prev.map((m) => m.id === assistantId ? { ...m, content: m.content + event.content! } : m));
            else if (event.type === "service_saved" && event.service) onServiceUpdate(event.service);
          } catch { /* skip */ }
        }
  ...
  }, [pendingConfirm, onServiceUpdate]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#fafafa" }}>
      <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid #e5e7eb" }}>
        <p style={{ fontSize: "0.7rem", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", margin: 0 }}>AI Үйлчилгээ оруулагч</p>
      </div>
      ...
      <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid #e5e7eb", background: "#ffffff" }}>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-end" }}>
          <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKey} disabled={isStreaming || !!pendingConfirm} placeholder={pendingConfirm ? "Баталгаажуулалтыг эхлээд шийднэ үү..." : "Үйлчилгээний нэр, хугацаа, үнэ хэлнэ үү..."} rows={2}
